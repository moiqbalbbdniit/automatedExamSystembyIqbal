  ...
  <div className="space-y-1 w-full">
    ...
    <input
      ...
      className="w-full rounded-xl border border-input bg-background/70 px-4 py-2 text-foreground placeholder:text-muted-foreground
             focus:ring-2 focus:ring-ring transition duration-150 ease-in-out"
    />
  </div>
);
...
export default function ChangePasswordPage() {
  ...
  return (
    <div className="min-h-screen aurora-page flex items-center justify-center px-4">
      <div className="panel w-full max-w-md space-y-6 p-6">
        ...
        <h1 className="text-2xl font-bold text-primary flex items-center gap-2">
          ...
          <button
      onClick={() => router.back()}
      className="text-primary hover:text-primary/80 cursor-pointer transition flex items-center gap-1 text-lg"
    >
      <ArrowLeft className="w-6 h-6" /> Back 
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-2 px-6 py-3 font-semibold rounded-xl 
                      ...
                      ${loading ? "bg-muted cursor-not-allowed" : "bg-primary hover:bg-primary/85 shadow-lg"}`}
        >
